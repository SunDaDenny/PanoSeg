...
import config
sys.path.append('../../Dataset/COCO2014/coco/PythonAPI')
import numpy as np
import setproctitle
import cv2
...
if __name__ == '__main__':
    ...
    for _ in range(len(ret_masks)):
        if ret_scores[_] > args.threshold and reserved[_]:
            ...
            if np.count_nonzero(mask) > max_result :
                mask[mask > 0] = 1.0
                mask_img = np.zeros([ mask.shape[0], mask.shape[1], 1], dtype=np.uint8)
                mask_img[:,:,0] = 255 * mask[:,:]
                cv2.imwrite( args.out ,mask_img)
            
    image = image.astype(np.uint8)
    #cv2.imshow('image', image)
    #cv2.imwrite("results/image_seg.jpg",image)
    #cv2.waitKey(100000)
