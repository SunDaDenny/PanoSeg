...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        if(outfile){
            ...
            char *joined;
            asprintf(&joined, "%s/%s", outfile , array[i-1]);

            save_image(im, joined);
            output_detection_bbox(im, l.w*l.h*l.n, thresh, boxes, probs, names, l.classes, joined);
        }
        else{
            save_image(im, "predictions");
            output_detection_bbox(im, l.w*l.h*l.n, thresh, boxes, probs, names, l.classes, "predictions");
#ifdef OPENCV
